...
public class UtilLoc {
    public static Location getAverage(List<Location> locations) {
    ...
    public static <Y extends Entity> Y getClosest(Location point, List<Y> entities, int minDistance, Entity... ignore) {
        Y closest = null;
        double dist = 0;
        ...
        for (Y entity : entities) {
            ...
            closest = entity;
            dist = d; // DAE bad programmer and assign to the wrong variable???
        }
    ...
    public static double getDistance(Location loc1, Location loc2) {
        return loc1.getWorld() == loc2.getWorld() ? loc1.distance(loc2) : loc1.toVector().distance(loc2.toVector());
    }
    ...
    public static Vector getVector(double yaw, double pitch) {
        ...
        vector.setY(-Math.sin(pitchRad));

        double xz = Math.cos(pitchRad);

        vector.setX(-xz * Math.sin(yawRad));
        vector.setZ(xz * Math.cos(yawRad));
