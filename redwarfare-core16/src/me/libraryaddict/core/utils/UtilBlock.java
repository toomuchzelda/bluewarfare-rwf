...
//        setPartiallySolid(Material.ENCHANTING_TABLE, true);
...
//        setPartiallySolid(Material.SPRUCE_STAIRS, true);
//        setPartiallySolid(Material.BIRCH_STAIRS, true);
//        setPartiallySolid(Material.JUNGLE_STAIRS, true);
//        setPartiallySolid(Material.COBBLESTONE_WALL, true);
...
//        setNonSolid(Material.TALL_GRASS);
//        setNonSolid(Material.DEAD_BUSH);
//        setNonSolid(Material.DANDELION);
//        setNonSolid(Material.POPPY);
...
//        setInteractable(Material.ENCHANTING_TABLE);
...
//    }
    ...
    public static List<Block> getBlocks(Location center, double sphereSize) {
        ArrayList<Block> blocks = new ArrayList<>();

        World world = center.getWorld();
        int originX = center.getBlockX(), originY = center.getBlockY(), originZ = center.getBlockZ();
        for (int x = (int) Math.floor(-sphereSize); x <= Math.ceil(sphereSize); x++) {
            for (int y = (int) Math.floor(-sphereSize); y <= Math.ceil(sphereSize); y++) {
                for (int z = (int) Math.floor(-sphereSize); z <= Math.ceil(sphereSize); z++) {
                    if (x * x + y * y + z * z > sphereSize * sphereSize)
                        continue;
...
//
//        while (nonSolid()[block.getType().ordinal()] && block.getY() > 0) {
...
//    }
    ...
    // "non-solid" seems to refer to blocks that don't have a hitbox
...
//    }
    ...
    public static boolean spawnable(Material mat) {
        return solid(mat) && mat.isOccluding();
    }
