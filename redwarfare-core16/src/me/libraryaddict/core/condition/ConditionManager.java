...
public class ConditionManager extends MiniPlugin {
    ...
    public static void addPotion(PotionEffect effect, Entity entity, Entity cause, boolean extend) {
        ...
        if (extend) {
            ...
            if (oldEffect != null) {
                ...
                effect = new PotionEffect(effect.getType(), effect.getDuration() + oldEffect.getDuration(),
                        Math.max(effect.getAmplifier(), oldEffect.getAmplifier()), effect.isAmbient(),
    ...
    public static void setFire(Entity entity, AttackType attackType, Entity cause, int ticks, boolean extend) {
        ...
        final int ticksRun = ticks;
        
        //apply the fire one tick later because for bows doing it on the same tick doesnt work for some reason as of 1.16.5
        if(attackType == AttackType.FIRE_BOW)
        {
	        new BukkitRunnable() {
	            public void run() {
	            	entity.setFireTicks(ticksRun);
	            }
	        }.runTaskLater(CentralManager.getManager().getCondition().getPlugin(), 1);
        }
        else
        {
