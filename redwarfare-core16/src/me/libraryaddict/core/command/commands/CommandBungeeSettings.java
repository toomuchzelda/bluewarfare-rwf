            ...
            public void run() {
                ...
                switch (args[0].toLowerCase()) {
                    ...
                    case "whitelist":
                        ...
                        break;
                  /*  case "favicon":		didn't work. always threw exception on constructing BufferedImage
                    	
                    	BufferedImage img = null;
                    	String filename = "./plugins/" + value;
                    	if(value != null) {
	                    	try {
	                    	...
	                    	{
	                    		UtilPlayer.sendMessage(player, UtilError.format(e.getMessage()));
	                    		
	                    		return;
	                    	}
	                    	catch(NullPointerException np)
	                    	{
	                    		Bukkit.broadcastMessage("pathname is null");
	                    	}
	                    	settings.setFavicon(value);
                    	}
                    	else {
                    		settings.setFavicon("");
                    		UtilPlayer.sendMessage(player, C.Blue + "Removed server icon");
                    	}
