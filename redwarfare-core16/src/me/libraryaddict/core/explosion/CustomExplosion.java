...
public class CustomExplosion extends Explosion {
    ...
    public CustomExplosion(Location loc, float explosionSize, AttackType attackType) {
        ...
        super(((CraftWorld) loc.getWorld()).getHandle(), null, null, null, loc.getX(), loc.getY(), loc.getZ(),
                explosionSize, false, Effect.NONE);
    ...
    public void a() {
                ...
                for (j = 0; j < 16; ++j) {
                    if (k == 0 || k == 15 || i == 0 || i == 15 || j == 0 || j == 15) {
                        ...
                        for (float f1 = 0.3F; f > 0.0F; f -= 0.22500001F) {
                            ...
                            Fluid fluid = this._world.getFluid(blockposition);
                            Optional<Float> optional = this.explosionDamageCalculator.a(this, this._world, blockposition, iblockdata, fluid);

                            if (optional.isPresent()) {
                                f -= ((Float) optional.get() + 0.3F) * 0.3F;
                            }

                            if (f > 0.0F && this.explosionDamageCalculator.a(this, this._world, blockposition, iblockdata, f)) {
        ...
        List<Entity> list = this._world.getEntities(this.source, new AxisAlignedBB((double) i, (double) l, (double) j1, (double) j, (double) i1, (double) k1));
        ...
        for (int l1 = 0; l1 < list.size(); ++l1) {
            ...
            double d7 = (double) (MathHelper.sqrt(entity.e(vec3d)) / this._size);
            ...
            if (d7 <= 1.0D) {
            	...
            	if (d11 != 0.0D) {
            		...
            		if(((entity instanceof EntityHuman)) && (!((EntityHuman) entity).abilities.isInvulnerable)) {
            			this.c().put((EntityHuman) entity, new Vec3D(d8 * d13, d9 * d13, d10 * d13));
            		}
                }
            }
        }
...
//            } end of comment block here
            ...
            for (BlockPosition blockposition : this.getBlocks()) {
...
//                                _blockExplosionSize, 0);
        ...
        if (this._createFire) {
            ...
            for (BlockPosition blockposition : this.getBlocks()) {
                if (this._world.getType(blockposition).isAir() && random.nextInt(3) == 0 &&
                        // what the fuck is i
                        // copied from 1.16 Explosion
                        this._world.getType(blockposition.down()).i(this._world, blockposition.down())) {
