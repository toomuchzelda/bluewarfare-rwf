...
public abstract class CentralManager extends MiniPlugin {
                ...
                new PacketAdapter(plugin, PacketType.Play.Server.REL_ENTITY_MOVE,
                    ...
                    public void onPacketSending(PacketEvent event) {
                        ...
                        if (event.getPacketType() == PacketType.Play.Server.ENTITY_TELEPORT) {
                            ...
                            StructureModifier<Short> pInts = packet.getShorts();
                            StructureModifier<Short> ints = toSend.getShorts();
    ...
    public PacketContainer[] getFakePlayerSpawn(UUID uuid, int entityId, Vector location) {
        ...
        List<PacketContainer> packets = new ArrayList<>(8);
        Collections.addAll(packets, writeDoubles(fakeIds, spawnPig));
        // add invisible packets too
        for (int fakeId : fakeIds) {
            packets.add(UtilEnt.getInvisPacket(fakeId));
        }

        return packets.toArray(new PacketContainer[0]);
    }
    ...
    //removed for using getHit method
   /* @EventHandler
