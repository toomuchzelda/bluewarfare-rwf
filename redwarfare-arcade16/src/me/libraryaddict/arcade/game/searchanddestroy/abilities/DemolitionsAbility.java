...
public class DemolitionsAbility extends Ability {
    ...
    public void onBlockPlace(BlockPlaceEvent event) {
        ...
        Material spawn = event.getBlock().getRelative(BlockFace.DOWN).getType();
        if (spawn == Material.INFESTED_CHISELED_STONE_BRICKS ||
                spawn == Material.INFESTED_COBBLESTONE ||
                spawn == Material.INFESTED_CRACKED_STONE_BRICKS ||
                spawn == Material.INFESTED_MOSSY_STONE_BRICKS ||
                spawn == Material.INFESTED_STONE ||
                spawn == Material.INFESTED_STONE_BRICKS) {
            return;
        }
    ...
    public void onInteract(PlayerInteractEvent event) {
        ...
        for (Landmine mine : _mines) {
            ...
            UtilParticle.playParticle(Particle.SMOKE_LARGE, mine.getLocation().add(0, -.5, 0), 0.3F, 0.0F, 0.3F, 0, 3);
    ...
    public void onTick(TimeEvent event) {
        ...
        for (Landmine mine : new ArrayList<Landmine>(_mines)) {
            if (mine.isArmed()) {
                ...
                CustomExplosion mineExplosion = new CustomExplosion(mine.getLocation(), 6F, DEMOLITIONS_EXPLODE);

                mineExplosion.setDamager(mine.getPlayer());

                mineExplosion.setMaxDamage(30f);

                mineExplosion.setDamageBlocks(false);
