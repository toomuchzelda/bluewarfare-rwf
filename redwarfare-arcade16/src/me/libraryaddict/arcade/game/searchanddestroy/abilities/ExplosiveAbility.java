...
public class ExplosiveAbility extends Ability {
    ...
    private void explode(Projectile projectile) {
        ...
        CustomExplosion rocket = new CustomExplosion(projectile.getLocation().subtract(projectile.getLocation().getDirection().normalize().multiply(0.1)),
                6.5F, RPG, RPG_SELF);

        rocket.setDamageBlocks(false);

        rocket.setMaxDamage(20f);

        rocket.setDamager(thrower);
    ...
    public void onGrenadeTick(TimeEvent event) {
        ...
        while (itel.hasNext()) {
            ...
            CustomExplosion nade = new CustomExplosion(entity.getLocation().add(0, 0.2, 0),
                    isAlive(entry.getValue().getValue()) ? 5.5F : 0, GRENADE);

            nade.setDamageBlocks(false);

            nade.setMaxDamage(7f);

            nade.setDamager(entry.getValue().getValue())
    ...
    public void onRPGTick(TimeEvent event) {
        ...
        while (itel.hasNext()) {
            ...
            UtilParticle.playParticle(Particle.EXPLOSION_NORMAL, egg.getLocation(), ViewDist.LONG);
        }
    }
