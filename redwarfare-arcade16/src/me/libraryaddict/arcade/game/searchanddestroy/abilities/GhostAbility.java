            ...
            public void onPacketSending(PacketEvent event) {
                try {
                    ...
                    if (event.getPacketType() == PacketType.Play.Server.NAMED_SOUND_EFFECT) {
                        Sound effect = event.getPacket().getSoundEffects().read(0);
                    ...
                    // entity meta needs to be changed with ENTITY_METADATA packet
                    // actually, send more packets to ensure invisibility
                    if (event.getPacketType() == PacketType.Play.Server.NAMED_ENTITY_SPAWN) {
                        PacketContainer container = UtilEnt.getInvisPacket(player);
                    ...
                    for(long i = 1; i < 11; i++)
                    {
                    	new BukkitRunnable() {
                            ...
                            public void run() {
                            	for (int i = 0; i < numParticles; i++) {
                                    UtilParticle.playParticle(player.getLocation()
                                            .add(UtilMath.rr(-0.3, .3), UtilMath.rr(0.4, 0.95), UtilMath.rr(-0.3, .3)), color, array);
                                }
                            }
