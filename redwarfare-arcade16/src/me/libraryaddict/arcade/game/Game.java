...
public abstract class Game implements Listener {
	...
	//
	//            if (validBlocks[block.getRelative(proper).getType().ordinal()]) {
	//                return proper;
	//            }
	//
	//            for (BlockFace face : new BlockFace[]{BlockFace.EAST, BlockFace.SOUTH, BlockFace.NORTH, BlockFace.WEST}) {
	//                if (validBlocks[block.getRelative(face).getType().ordinal()]) {
	//                    return face;
	//                }
	//            }
	//        }
	...
	//give admin/owner 1 kill if typed "killstreakme". for testing killstreaks and stuff.
	//(must have admin or higher rank set to display in chat)
	//should prob make this a real command
	@EventHandler
	public void onChat(ChatEvent event) {
		if(event.getMessage()[0].startsWith("kb"))
		{
			...
			else if(event.getMessage()[0].equalsIgnoreCase("kbreduce"))
			{
				...
				Bukkit.broadcastMessage(C.Blue + "Using reduceVelocity():" + CustomDamageEvent.reduceVel);
				event.setCancelled(true);
			}
			...
			else if(event.getMessage()[0].equalsIgnoreCase("kbaw"))
			{
				CustomDamageEvent.useAv = !CustomDamageEvent.useAv;
				Bukkit.broadcastMessage(C.Blue + "Using av instead of aw: " + CustomDamageEvent.useAv);
				event.setCancelled(true);
			}
			else if(event.getMessage()[0].equalsIgnoreCase("kbimpulse"))
			{
				CustomDamageEvent.useImpulse = !CustomDamageEvent.useImpulse;
				Bukkit.broadcastMessage(C.Blue + "Using setting impulse: " + CustomDamageEvent.useImpulse);
				event.setCancelled(true);
			}
			...
			{
				event.setCancelled(true);
				String[] split = event.getMessage()[0].split(" ");

				if(split.length == 2)
				{
					if(split[1].equalsIgnoreCase("mult"))
					{
						event.getPlayer().sendMessage(C.Red + "Put a number after mult");
						return;
					}

					try
					{
						CustomDamageEvent.yVal = Double.parseDouble(split[1]);
						Bukkit.broadcastMessage(C.Blue + "Set kb y to " + CustomDamageEvent.yVal);
					}
					catch(Exception e)
					{
						event.getPlayer().sendMessage(C.Red + e.getMessage() + " Input a valid number");
						return;
					}
				}
				else if(split.length == 3)
				{
					if(split[1].equalsIgnoreCase("mult"))
					{
						try
						{
							CustomDamageEvent.yMult = Double.parseDouble(split[2]);
							Bukkit.broadcastMessage(C.Blue + "Set kb mult y adder to " + CustomDamageEvent.yMult);
						}
						catch(Exception e)
						{
							event.getPlayer().sendMessage(C.Red + e.getMessage() + " Input a valid number");
					...
					{
						event.getPlayer().sendMessage(C.Red + "Bad args");
					}
				}
			}
			else if(event.getMessage()[0].toLowerCase().startsWith("kbxz"))
			{
				event.setCancelled(true);
				String[] split = event.getMessage()[0].split(" ");
				if(split.length == 2)
				{        	
					...
					try
					{
						CustomDamageEvent.xzVal = Double.parseDouble(split[1]);
						Bukkit.broadcastMessage(C.Blue + "Set kb xz to " + CustomDamageEvent.xzVal);
					}
					catch(Exception e)
					{
						event.getPlayer().sendMessage(C.Red + e.getMessage() + " Input a valid number");
				...
				else if(split.length == 3)
				{
					if(split[1].equalsIgnoreCase("mult"))
					{
						try
						{
							CustomDamageEvent.xzMult = Double.parseDouble(split[2]);
							Bukkit.broadcastMessage(C.Blue + "Set kb multiplier xz to " + CustomDamageEvent.xzMult);
						}
						catch(Exception e)
						{
							event.getPlayer().sendMessage(C.Red + e.getMessage() + " Input a valid number");
		...
		else if (event.getRank().isBetterRating(Rank.MOD))
		{
			if(event.getMessage()[0].equalsIgnoreCase("killstreakme"))
			{
				event.setCancelled(true);

				giveKillstreak(event.getPlayer(),
						new DeathEvent(event.getPlayer(), getManager().getCombat()
								.getCreateCombatLog(event.getPlayer())), 1);
			}
			else if(event.getMessage()[0].equalsIgnoreCase("givealljumper"))
			{
				event.setCancelled(true);
	...
	public void spawnDeadBody(Player deadPlayer) {
		Bukkit.broadcastMessage("Dead body to be spawned for " + deadPlayer.getName());
		/*
