...
public class LobbyManager extends MiniPlugin {
    ...
    private HashMap<ItemStack, GameTeam> joinTeamItems = new HashMap<>();
    private boolean _waitingForTeles;
    private long _waitingSince;
    private ArrayList<ForceKitCanidate> canidates = new ArrayList<>();
    ...
    public void generateTeamItems() {
        joinTeamItems.clear();
        getGame().getTeams().forEach(team -> {
            TeamSettings settings = team.getSettings();
            ItemBuilder builder = new ItemBuilder(settings.getHat());
            builder.setTitle(settings.getColor() + "Join " + settings.getName());
    ...
    public int getMinPlayers() {
        ...
        long elasped = Math.floorDiv(System.currentTimeMillis() - _waitingSince, (long) 15000);
    ...
    @EventHandler
    public void onInteractTeams(PlayerInteractEvent event) {
        ItemStack item = event.getItem();
        ...
        GameTeam team = joinTeamItems.get(item);
        if (team != null) {
            // im very lazy
            player.chat("/team " + team.getSettings().getSimpleName());
        }
    }
    ...
    public void onJoin(PlayerJoinEvent event) {
        ...
        if (getGame().getState() == GameState.PreMap && getGame().getOption(GameOption.MAP_VOTE)) {
            player.getInventory().setItem(8, _mapVote);
        } else if (getGame().getState() == GameState.MapLoaded) {
            generateTeamItems();
            joinTeamItems.keySet().forEach(item -> UtilInv.addItem(player, item));
        }
    ...
    public void onMapFinishedLoading(GameStateEvent event) {
        if (event.getState() != GameState.MapLoaded) {
            return;
        }
    ...
    @EventHandler(priority = EventPriority.LOWEST)
    public void onGameAlmostStart(GameStateEvent event) {
        if (event.getState() != GameState.PrepareGame) {
            return;
        }

        for (Player player : Bukkit.getOnlinePlayers()) {
            joinTeamItems.keySet().forEach(item -> UtilInv.remove(player, item));
        }
