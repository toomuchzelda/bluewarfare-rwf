...
public class EventManager extends MiniPlugin {
    ...
    //fix problem where ender pearls hitting entities aren't teleporting the thrower
    @EventHandler
    public void onProjectileLand(ProjectileHitEvent event)
    {
    	//if the projectile is a pearl and it hit an entity and the thrower is an entity then teleport thrower
    	if (event.getHitEntity() != null && event.getEntity().getType() == EntityType.ENDER_PEARL)
    	{
    		if(event.getEntity().getShooter() instanceof LivingEntity)
    		{
    			//get the thrower of the pearl
    			LivingEntity living = (LivingEntity) event.getEntity().getShooter();
    			
    			//get the location where the thrower will be teleported (The ender pearl) but keep the thrower's current looking direction
    			Location loc = event.getEntity().getLocation();
    			loc.setPitch(living.getLocation().getPitch());
    			loc.setYaw(living.getLocation().getYaw());
    			
    			//teleport the thrower with the ender pearl set as cause
    			living.teleport(loc, PlayerTeleportEvent.TeleportCause.ENDER_PEARL);
    		}
    	}
    }
