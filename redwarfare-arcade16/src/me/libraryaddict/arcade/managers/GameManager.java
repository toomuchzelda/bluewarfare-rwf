...
public class GameManager extends MiniPlugin {
                ...
                try {
                    ...
                    if (event.getPacketType() == PacketType.Play.Server.NAMED_ENTITY_SPAWN) {
                        ...
                        for (EquipmentSlot slot : EquipmentSlot.values()) {
                            ...
                            PacketContainer newPacket = UtilEnt.getEquipmentPacket(player, slot, equipEvent.getItem());
                        ...
                        List<com.comphenix.protocol.wrappers.Pair<EnumWrappers.ItemSlot, ItemStack>> list = packet.getSlotStackPairLists().read(0);
                        // use iterator to change immutable pair
                        ListIterator<com.comphenix.protocol.wrappers.Pair<EnumWrappers.ItemSlot, ItemStack>> iterator = list.listIterator();
                        while (iterator.hasNext()) {
                            com.comphenix.protocol.wrappers.Pair<EnumWrappers.ItemSlot, ItemStack> pair = iterator.next();
                            ...
                            } else if (slot == EnumWrappers.ItemSlot.OFFHAND) {
                                bukkitSlot = EquipmentSlot.OFF_HAND;
                            } else {
                                bukkitSlot = EquipmentSlot.valueOf(slot.name());
                            }
                            ...
                            iterator.set(new com.comphenix.protocol.wrappers.Pair<>(pair.getFirst(), hatEvent.getItem()));
                        }
    ...
    private void balanceTeams() {
        ...
        ArrayList<Pair<GameTeam, Integer>> teamBalance = new ArrayList<>();
        ArrayList<Player> specialKits = new ArrayList<>();
        ...
        while (!specialKits.isEmpty() && !teamBalance.isEmpty()) {
            teamBalance.sort(Comparator.comparingInt(Pair::getValue));
                ...
                for (Player sendPlayer : UtilPlayer.getPlayers()) {
                    ...
                    for (EquipmentSlot slot : EquipmentSlot.values()) {
                        try {
                            ...
                            ProtocolLibrary.getProtocolManager().sendServerPacket(receivePacket, UtilEnt.getEquipmentPacket(sendPlayer, slot, item));
